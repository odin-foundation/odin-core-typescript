...
describe('evaluateCondition', () => {
  ...
  describe('compound conditions (and/or/not)', () => {
    ...
    it('and: true when all factors are true', () => {
      const r = createResolver({ hasDui: bool(true), age: int(20) });
      expect(evaluateCondition('@.hasDui = true and @.age < 25', ctx, r)).toBe(true);
    });

    it('and: false when one factor is false', () => {
      const r = createResolver({ hasDui: bool(true), age: int(40) });
      expect(evaluateCondition('@.hasDui = true and @.age < 25', ctx, r)).toBe(false);
    });

    it('or: true when any term is true', () => {
      const r = createResolver({ state: str('NY') });
      expect(evaluateCondition("@.state = 'CA' or @.state = 'NY'", ctx, r)).toBe(true);
    });

    it('or: false when all terms are false', () => {
      const r = createResolver({ state: str('TX') });
      expect(evaluateCondition("@.state = 'CA' or @.state = 'NY'", ctx, r)).toBe(false);
    });

    it('not: negates a truthy check', () => {
      const r = createResolver({ verified: bool(false) });
      expect(evaluateCondition('not @.verified', ctx, r)).toBe(true);
    });

    it('not: negates a comparison', () => {
      const r = createResolver({ status: str('active') });
      expect(evaluateCondition("not @.status = 'closed'", ctx, r)).toBe(true);
    });

    it('not binds tighter than and', () => {
      const r = createResolver({ a: bool(false), b: bool(true) });
      expect(evaluateCondition('not @.a and @.b', ctx, r)).toBe(true);
    });

    it('and binds tighter than or', () => {
      const rFalse = createResolver({ a: bool(false), b: bool(true), c: bool(false) });
      expect(evaluateCondition('@.a or @.b and @.c', ctx, rFalse)).toBe(false);
      const rTrue = createResolver({ a: bool(true), b: bool(false), c: bool(false) });
      expect(evaluateCondition('@.a or @.b and @.c', ctx, rTrue)).toBe(true);
    });

    it('chains three and-factors', () => {
      const r = createResolver({ a: bool(true), b: bool(true), c: bool(true) });
      expect(evaluateCondition('@.a and @.b and @.c', ctx, r)).toBe(true);
    });

    it('does not split operator words inside quoted values', () => {
      const r = createResolver({ desc: str('cats and dogs') });
      expect(evaluateCondition("@.desc = 'cats and dogs'", ctx, r)).toBe(true);
    });

    it('is case-insensitive for operators', () => {
      const r = createResolver({ a: bool(true), b: bool(true) });
      expect(evaluateCondition('@.a AND @.b', ctx, r)).toBe(true);
    });
  });
});
